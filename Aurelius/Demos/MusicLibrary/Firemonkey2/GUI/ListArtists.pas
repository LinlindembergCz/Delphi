...
type
  ...
  private
    FGridController: TGridController<TArtist>;
...
procedure TfrmListArtists.DestroyGridController;
begin
...
//  BindScope1.Active := True;
  ...
  DestroyGridController;
  FGridController := TGridController<TArtist>.Create(Grid, FController.GetAllArtists,
    procedure(Obj: TArtist; Cols: TArray<string>)
    begin
      Cols[0] := Obj.ArtistName;
      if Obj.Genre.HasValue then
        Cols[1] := Obj.Genre;
    end);
end;
